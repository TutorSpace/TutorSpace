...
<div class="{{ $hidden ? 'hidden-2' : '' }}">
    <div class="info-box" data-route-url="{{ route('payment.stripe.refund.user_request_refund', $session->id) }}">
        ...
        <div class="date">
            ...
            <div>
                <span class="content">
                    {{ Carbon\Carbon::parse(date("m/d", strtotime($session->session_time_start)))->setTimeZone($tz)->format('m/d') }}<span class="info-box__year">/{{ Carbon\Carbon::parse(date("/y", strtotime($session->session_time_start)))->setTimeZone($tz)->format('y') }}</span>
                    {{ Carbon\Carbon::parse(date("D", strtotime($session->session_time_start)))->setTimeZone($tz)->format('D') }}
                </span>
            </div>
