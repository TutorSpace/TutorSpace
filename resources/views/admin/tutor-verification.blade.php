                ...
                <td>
                    <form action="{{ route('admin.course.post', App\User::find($notif->notifiable_id)) }}" method="POST" style="width: 100%">
                        ...
                        <div class="autocomplete d-flex">
                            <input type="text" placeholder="course name" class="form-control form-control-lg course fs-1-6" name="course" id="course-{{ $notif->notifiable_id }}" required>
                            <button class="btn btn-primary ml-3 fs-1-6" type="submit">Add</button>
                        </div>
                    </form>
...
$.ajax({
    ...
    success: (data) => {
        ...
        @foreach (Illuminate\Support\Facades\DB::table('notifications')->where('type', 'App\Notifications\TutorVerificationInitiatedNotification')->get() as $notif)
        autocomplete(document.getElementById("course-{{ $notif->notifiable_id }}"), courses);
        @endforeach
    }
});
</script>
